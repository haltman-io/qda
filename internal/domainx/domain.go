// Package domainx normalizes words, domains and TLDs and expands inputs
// into checkable targets.
package domainx
...
// NormalizeDomain converts URLs, hosts with ports, wildcard prefixes and
// Unicode domains into a canonical lower-case ASCII domain.
...
// RegistrableDomain reduces a domain to its registrable root using the
// Public Suffix List (example.co.uk stays example.co.uk).
...
// NormalizeTLD validates and canonicalizes a TLD or public suffix.
...
// NormalizeWord validates a bare word used to build domains.
...
// BuildDomainFromWord joins a word and a TLD into a normalized domain.
...
// IsBRDomain reports whether a domain ends in .br.
func IsBRDomain(domain string) bool {
	domain = strings.ToLower(strings.TrimSpace(domain))
	domain = strings.TrimSuffix(domain, ".")
	return strings.HasSuffix(domain, ".br")
}

// TLDOf returns the public suffix of a domain (best effort).
